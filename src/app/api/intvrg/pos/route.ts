/**
 * INTVRG POS Bilgileri Sorgulama API Endpoint
 * ============================================
 * Dashboard'dan Electron Bot'a POS sorgulama sinyali gönderir.
 *
 * POST /api/intvrg/pos
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.error("[intvrg-pos] Müşteri şifre çözümleme hatası:", decryptError);
      return NextResponse.json(
    ...
    try {
      ...
      console.warn("[intvrg-pos] Bot bağlantı kontrolü yapılamadı");
      return NextResponse.json(
        {
          error: "SMMM Asistan masaüstü uygulaması bağlı değil. Lütfen uygulamayı çalıştırın.",
          code: "BOT_NOT_CONNECTED",
    ...
    // 7. Bot'a intvrg:pos-query sinyali gönder
    ...
    return NextResponse.json({
      success: true,
      message: `POS sorgulaması başlatıldı (${customerName})`,
    });
  } catch (error) {
    console.error("[intvrg-pos] Beklenmeyen hata:", error);
    return NextResponse.json(
