/**
 * INTVRG ÖKC Bildirim Sorgulama API Endpoint
 * ============================================
 * Dashboard'dan Electron Bot'a ÖKC bildirim sorgulama sinyali gönderir.
 *
 * POST /api/intvrg/okc-bildirim
 * Body: { customerId: string, ay: string, yil: string }
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { customerId, ay, yil } = body;

    if (!customerId || !ay || !yil) {
      return NextResponse.json(
        { error: "customerId, ay ve yil zorunludur" },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      console.error("[intvrg-okc] Müşteri şifre çözümleme hatası:", decryptError);
      return NextResponse.json(
    ...
    try {
      ...
      console.warn("[intvrg-okc] Bot bağlantı kontrolü yapılamadı");
      return NextResponse.json(
        {
          error: "SMMM Asistan masaüstü uygulaması bağlı değil. Lütfen uygulamayı çalıştırın.",
          code: "BOT_NOT_CONNECTED",
    ...
    // 7. Bot'a intvrg:okc-query sinyali gönder
    ...
    return NextResponse.json({
      success: true,
      message: `ÖKC bildirim sorgulaması başlatıldı (${customerName})`,
    });
  } catch (error) {
    console.error("[intvrg-okc] Beklenmeyen hata:", error);
    return NextResponse.json(
