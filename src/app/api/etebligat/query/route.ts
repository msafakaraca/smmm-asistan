/**
 * E-Tebligat Sorgulama API Endpoint
 * ==================================
 * Dashboard'dan Electron Bot'a e-Tebligat sorgulama sinyali gönderir.
 *
 * POST /api/etebligat/query
...
interface EtebligatQueryRequest {
  customerId: string;
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    let body: EtebligatQueryRequest;
    try {
      body = await req.json();
    } catch {
    ...
    try {
      ...
      console.error("[etebligat-query] Müşteri şifre çözümleme hatası:", decryptError);
      return NextResponse.json(
    ...
    try {
      ...
      console.warn("[etebligat-query] Bot bağlantı kontrolü yapılamadı");
      return NextResponse.json(
        {
          error: "SMMM Asistan masaüstü uygulaması bağlı değil. Lütfen uygulamayı çalıştırın.",
          code: "BOT_NOT_CONNECTED",
    ...
    // 7. Bot'a etebligat:query sinyali gönder
    ...
    return NextResponse.json({
      success: true,
      message: `E-Tebligat sorgulaması başlatıldı (${customerName})`,
    });
  } catch (error) {
    console.error("[etebligat-query] Beklenmeyen hata:", error);
    return NextResponse.json(
