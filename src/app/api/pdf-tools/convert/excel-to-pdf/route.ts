...
/**
 * POST /api/pdf-tools/convert/excel-to-pdf
 *
 * Excel dosyasini PDF'e donustur.
 *
 * FormData:
 * - fileInput: File - Excel dosyasi (.xls, .xlsx, .ods, .csv)
 *
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!isFileTypeSupported(file.name, 'EXCEL')) {
      const supportedTypes = STIRLING_CONFIG.SUPPORTED_TYPES.EXCEL.join(', ');
    ...
    console.log(`[PDF-Tools] Excel to PDF: ${file.name}, boyut: ${(file.size / 1024).toFixed(2)}KB`);
    ...
    if (!response.ok) {
      ...
      console.error(`[PDF-Tools] Excel to PDF hatasi: ${response.status} - ${errorText}`);
      return NextResponse.json(
        { error: `Excel -> PDF donusturme basarisiz: ${errorText}` },
    ...
    console.log(`[PDF-Tools] Excel to PDF basarili: ${baseName}, ${(pdfBuffer.byteLength / 1024).toFixed(2)}KB`);
    ...
    if (error.name === 'AbortError') {
      console.error('[PDF-Tools] Excel to PDF timeout');
      return NextResponse.json(
    ...
    console.error('[PDF-Tools] Excel to PDF error:', error);
    return NextResponse.json(
      { error: "Excel -> PDF donusturme sirasinda bir hata olustu" },
      { status: 500 }
    );
  }
}
