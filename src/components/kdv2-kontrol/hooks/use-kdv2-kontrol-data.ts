/**
 * useKdv2KontrolData Hook
 *
 * KDV2 (Tevkifat) Kontrol sayfası için veri yönetimi hook'u.
...
interface UseKdv2KontrolDataOptions {
  initialYear?: number;
  initialMonth?: number;
}

export function useKdv2KontrolData(options: UseKdv2KontrolDataOptions = {}) {
  ...
  // Data states
  const [data, setData] = useState<Kdv2KontrolData[]>([]);
  ...
  // KDV2 Kontrol verilerini yükle
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(
        `/api/kdv2-kontrol?year=${selectedYear}&month=${selectedMonth}`
      );
      if (res.ok) {
        const result = await res.json();
        setData(result);
      } else {
        toast.error("KDV-2 kontrol verileri yüklenemedi");
      }
    } catch (error) {
      console.error("Error fetching KDV2 kontrol data:", error);
      toast.error("KDV-2 kontrol verileri yüklenirken hata oluştu");
    } finally {
      setLoading(false);
    }
    ...
    const fetchGroups = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // KDV2 beyannameTypes içeren grupları filtrele
          const kdv2Groups = data.filter((g: CustomerGroup) =>
            g.beyannameTypes?.includes('KDV2')
          );
          setGroups(kdv2Groups);
        }
      } catch (error) {
  ...
  // WebSocket bağlantısı - KDV2 parse event'lerini dinle
  useEffect(() => {
      ...
      try {
        ...
        ws.onmessage = (event) => {
          try {
            ...
            // KDV2 parse complete event
            if (message.type === "kdv2:parse-complete") {
              const data = message.data;
              console.log("[KDV2-WS] Parse complete:", data);

              setIsParsingAll(false);

              if (data?.success) {
                toast.success(data.message || "KDV-2 verileri parse edildi");
                // Tabloyu yenile
                fetchData();
              } else {
            ...
            // Bot progress event (KDV2 için)
            if (message.type === "BOT_PROGRESS") {
              const payload = message.payload;
              if (
                payload?.message?.toLowerCase().includes("kdv2") ||
                payload?.message?.toLowerCase().includes("tevkifat")
              ) {
                // KDV2 ile ilgili progress
            ...
            console.error("[KDV2-WS] Parse error", e);
          }
        };

        ws.onclose = () => {
          console.log("[KDV2-WS] Disconnected");
          reconnectTimerRef.current = setTimeout(connectWS, 5000);
        };
      } catch (e) {
        console.error("[KDV2-WS] Setup failed", e);
      }
    };
  ...
  const updateStatus = useCallback(
    async (customerId: string, status: Kdv2Status, notes?: string) => {
      ...
      try {
        const res = await fetch("/api/kdv2-kontrol", {
          method: "PUT",
  ...
  const parseCustomer = useCallback(
    async (customerId: string): Promise<Kdv2ParseResult | null> => {
      setIsParsing(true);
      try {
        const res = await fetch("/api/kdv2-kontrol/parse", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            customerId,
  ...
  const parseAll = useCallback(async (): Promise<Kdv2ParseAllResult | null> => {
    setIsParsingAll(true);
    try {
      const res = await fetch("/api/kdv2-kontrol/parse-all", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          year: selectedYear,
      ...
      if (res.ok && result.success) {
        // Bot'a mesaj gönderildi - WebSocket'ten kdv2:parse-complete event'i bekle
  ...
  const bulkUpdateStatus = useCallback(
    async (customerIds: string[], status: Kdv2Status): Promise<boolean> => {
      try {
        const res = await fetch("/api/kdv2-kontrol/bulk-status", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            customerIds,
  ...
  const clearAll = useCallback(async (): Promise<boolean> => {
    ...
    try {
      const res = await fetch(
        `/api/kdv2-kontrol/clear?year=${selectedYear}&month=${selectedMonth}`,
        { method: "DELETE" }
      );
