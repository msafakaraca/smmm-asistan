 ...
 *
 * type parametresi ile beyanname veya tahakkuk dosyalarını filtreler
 */
...
interface Kdv2FilePopoverProps {
  customerId: string;
  year: number;
  month: number;
...
export function Kdv2FilePopover({
  customerId,
  year,
  month,
  type,
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/kdv2-kontrol/customer-files?${params}`, {
  ...
  const handleTriggerClick = useCallback(async (e: React.MouseEvent) => {
    ...
    if (fileCount === 1) {
      ...
      try {
        ...
        console.error("[Kdv2FilePopover] Dosya açılamadı:", error);
    ...
    try {
      ...
      console.error("[Kdv2FilePopover] Dosyalar getirilemedi:", error);
      setNoFiles(true);
      setFiles([]);
    } finally {
      setLoading(false);
