...
/**
 * KDV9015 Kontrol Table Component
 *
 * KDV9015 (KDV Tevkifat) Tahakkuk bilgilerini gosteren tablo.
 */
...
import type { Kdv9015KontrolData, Kdv9015Status } from "./types";
import { Kdv9015FilePopover } from "./kdv9015-file-popover";

// Virtual scrolling threshold
const VIRTUAL_THRESHOLD = 100;

// Status -> Background color map (sabit, component disinda)
const STATUS_BG_MAP: Record<Kdv9015Status, string> = {
...
const STATUS_CONFIG: Record<Kdv9015Status, { icon: typeof CheckCircle; color: string; label: string }> = {
  verildi: {
    icon: CheckCircle,
...
interface Kdv9015KontrolTableProps {
  data: Kdv9015KontrolData[];
  onUpdateStatus: (customerId: string, status: Kdv9015Status, notes?: string) => void;
...
const StatusBadge = memo(function StatusBadge({ status }: { status: Kdv9015Status }) {
...
export const Kdv9015KontrolTable = memo(function Kdv9015KontrolTable({
  data,
  onUpdateStatus,
  onParseCustomer,
  isParsing,
  ...
  const handleOpenNote = (item: Kdv9015KontrolData) => {
                    ...
                    <td className="border border-border px-1 py-1">
                      <div className="flex items-center justify-center gap-0.5">
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent side="top">
                            <p>{item.notes ? 'Not Duzenle' : 'Not Ekle'}</p>
                          </TooltipContent>
                        </Tooltip>
