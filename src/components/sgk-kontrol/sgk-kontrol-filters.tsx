...
/**
 * SGK Kontrol Filters Component
 *
 * Dönem seçimi, arama ve filtreler.
 */

import { memo, useState } from "react";
import { Search, Calendar, Filter, RefreshCw, Trash2, Users, CheckCircle, AlertCircle, Clock, XCircle, FileCheck, ListFilter, CheckSquare } from "lucide-react";
...
interface SgkKontrolFiltersProps {
  selectedYear: number;
  ...
  stats: {
    ...
    eksik: number;
    bekliyor: number;
    gonderilmeyecek: number;
    dilekce_gonderildi: number;
  };
...
export const SgkKontrolFilters = memo(function SgkKontrolFilters({
  selectedYear,
  setSelectedYear,
  selectedMonth,
  setSelectedMonth,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-6 gap-3 p-0.5">
        <button
          ...
          className={`bg-slate-100 border border-slate-300 rounded-lg p-3 text-left transition-all cursor-pointer hover:shadow-md ${
            activeCardFilter === null ? "ring-2 ring-primary shadow-md" : ""
          }`}
        >
        ...
        <button
          onClick={() => setActiveCardFilter(activeCardFilter === "gonderildi" ? null : "gonderildi")}
          className={`bg-green-100 border border-green-300 rounded-lg p-3 text-left transition-all cursor-pointer hover:shadow-md ${
            activeCardFilter === "gonderildi" ? "ring-2 ring-green-500 shadow-md" : ""
          }`}
        >
          <div className="text-xl font-bold text-green-800">
            {stats.gonderildi}
          </div>
          <div className="text-xs text-green-700">Gönderildi</div>
        </button>
        <button
        ...
        <button
          onClick={() => setActiveCardFilter(activeCardFilter === "gonderilmeyecek" ? null : "gonderilmeyecek")}
          className={`bg-red-100 border border-red-300 rounded-lg p-3 text-left transition-all cursor-pointer hover:shadow-md ${
            activeCardFilter === "gonderilmeyecek" ? "ring-2 ring-red-500 shadow-md" : ""
          }`}
        >
          <div className="text-xl font-bold text-red-800">
            {stats.gonderilmeyecek}
          </div>
          <div className="text-xs text-red-700">Gönderilmeyecek</div>
        </button>
        <button
          onClick={() => setActiveCardFilter(activeCardFilter === "dilekce_gonderildi" ? null : "dilekce_gonderildi")}
          className={`bg-purple-100 border border-purple-300 rounded-lg p-3 text-left transition-all cursor-pointer hover:shadow-md ${
            activeCardFilter === "dilekce_gonderildi" ? "ring-2 ring-purple-500 shadow-md" : ""
          }`}
        >
          <div className="text-xl font-bold text-purple-800">
            {stats.dilekce_gonderildi}
          </div>
          <div className="text-xs text-purple-700">Dilekçe Gönderildi</div>
        </button>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          ...
          <SelectContent>
            ...
            <SelectItem value="gonderildi">
              <span className="flex items-center gap-2">
                <CheckCircle className="h-4 w-4 text-green-600" />
                <span className="text-green-700">Gönderildi</span>
              </span>
            </SelectItem>
            ...
            <SelectItem value="gonderilmeyecek">
              <span className="flex items-center gap-2">
                <XCircle className="h-4 w-4 text-red-600" />
                <span className="text-red-700">Gönderilmeyecek</span>
              </span>
            </SelectItem>
            <SelectItem value="dilekce_gonderildi">
              <span className="flex items-center gap-2">
                <FileCheck className="h-4 w-4 text-purple-600" />
                <span className="text-purple-700">Dilekçe Gönderildi</span>
              </span>
            </SelectItem>
          </SelectContent>
        </Select>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              tüm SGK kontrol verileri silinecek. Bu işlem geri alınamaz.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
