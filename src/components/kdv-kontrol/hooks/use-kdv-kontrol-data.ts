/**
 * useKdvKontrolData Hook
 *
 * KDV Kontrol sayfası için veri yönetimi hook'u.
...
interface UseKdvKontrolDataOptions {
  initialYear?: number;
  initialMonth?: number;
}

export function useKdvKontrolData(options: UseKdvKontrolDataOptions = {}) {
  ...
  // Data states
  const [data, setData] = useState<KdvKontrolData[]>([]);
  ...
  // KDV Kontrol verilerini yükle
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(
        `/api/kdv-kontrol?year=${selectedYear}&month=${selectedMonth}`
      );
      if (res.ok) {
        const result = await res.json();
        setData(result);
      } else {
        toast.error("KDV kontrol verileri yüklenemedi");
      }
    } catch (error) {
      console.error("Error fetching KDV kontrol data:", error);
      toast.error("KDV kontrol verileri yüklenirken hata oluştu");
    } finally {
      setLoading(false);
    }
    ...
    const fetchGroups = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // KDV1 beyannameTypes içeren grupları filtrele
          const kdv1Groups = data.filter((g: CustomerGroup) =>
            g.beyannameTypes?.includes('KDV1')
          );
          setGroups(kdv1Groups);
        }
      } catch (error) {
  ...
  // WebSocket bağlantısı - KDV parse event'lerini dinle
  useEffect(() => {
      ...
      try {
        ...
        ws.onmessage = (event) => {
          try {
            ...
            // KDV parse complete event
            if (message.type === "kdv:parse-complete") {
              const data = message.data;
              console.log("[KDV-WS] Parse complete:", data);

              setIsParsingAll(false);

              if (data?.success) {
                toast.success(data.message || "KDV verileri parse edildi");
                // Tabloyu yenile
                fetchData();
              } else {
            ...
            // Bot progress event (KDV için)
            if (message.type === "BOT_PROGRESS") {
              const payload = message.payload;
              if (
                payload?.message?.toLowerCase().includes("kdv") ||
                payload?.message?.toLowerCase().includes("tahakkuk")
              ) {
                // KDV ile ilgili progress
            ...
            console.error("[KDV-WS] Parse error", e);
          }
        };

        ws.onclose = () => {
          console.log("[KDV-WS] Disconnected");
          reconnectTimerRef.current = setTimeout(connectWS, 5000);
        };
      } catch (e) {
        console.error("[KDV-WS] Setup failed", e);
      }
    };
  ...
  const updateStatus = useCallback(
    async (customerId: string, status: KdvStatus, notes?: string) => {
      ...
      try {
        const res = await fetch("/api/kdv-kontrol", {
          method: "PUT",
  ...
  const parseCustomer = useCallback(
    async (customerId: string): Promise<KdvParseResult | null> => {
      setIsParsing(true);
      try {
        const res = await fetch("/api/kdv-kontrol/parse", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            customerId,
  ...
  const parseAll = useCallback(async (): Promise<KdvParseAllResult | null> => {
    setIsParsingAll(true);
    try {
      const res = await fetch("/api/kdv-kontrol/parse-all", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          year: selectedYear,
      ...
      if (res.ok && result.success) {
        // Bot'a mesaj gönderildi - WebSocket'ten kdv:parse-complete event'i bekle
  ...
  const bulkUpdateStatus = useCallback(
    async (customerIds: string[], status: KdvStatus): Promise<boolean> => {
      try {
        const res = await fetch("/api/kdv-kontrol/bulk-status", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            customerIds,
  ...
  const clearAll = useCallback(async (): Promise<boolean> => {
    ...
    try {
      const res = await fetch(
        `/api/kdv-kontrol/clear?year=${selectedYear}&month=${selectedMonth}`,
        { method: "DELETE" }
      );
