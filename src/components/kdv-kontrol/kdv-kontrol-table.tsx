...
/**
 * KDV Kontrol Table Component
 *
 * KDV1 Tahakkuk bilgilerini gösteren tablo.
 */
...
import type { KdvKontrolData, KdvStatus } from "./types";
import { KdvFilePopover } from "./kdv-file-popover";

// Virtual scrolling threshold
const VIRTUAL_THRESHOLD = 100;

// Status -> Background color map (sabit, component dışında)
const STATUS_BG_MAP: Record<KdvStatus, string> = {
...
const STATUS_CONFIG: Record<KdvStatus, { icon: typeof CheckCircle; color: string; label: string }> = {
  verildi: {
    icon: CheckCircle,
...
interface KdvKontrolTableProps {
  data: KdvKontrolData[];
  onUpdateStatus: (customerId: string, status: KdvStatus, notes?: string) => void;
...
const StatusBadge = memo(function StatusBadge({ status }: { status: KdvStatus }) {
...
export const KdvKontrolTable = memo(function KdvKontrolTable({
  data,
  onUpdateStatus,
  onParseCustomer,
  isParsing,
  ...
  const handleOpenNote = (item: KdvKontrolData) => {
        ...
        >
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {paddingTop > 0 && (
                <tr>
                  <td style={{ height: paddingTop }} colSpan={isSelectionMode ? 15 : 14} />
                </tr>
              )}
              {rowsToRender.map((virtualRow) => {
                ...
                const hasDevredenKdv = (item.devredenKdv ?? 0) > 0;
                ...
                return (
                  ...
                  >
                    ...
                    <td className={`border border-border px-3 py-2 text-right text-xs font-bold ${
                      hasDevredenKdv ? 'text-blue-700 bg-blue-50' : ''
                    }`}>
                      {formatCurrency(item.devredenKdv)}
                    </td>
                    <td className={`border border-border px-3 py-2 text-right text-xs font-bold ${
              ...
              {paddingBottom > 0 && (
                <tr>
                  <td style={{ height: paddingBottom }} colSpan={isSelectionMode ? 15 : 14} />
                </tr>
              )}
            </tbody>
          </table>
