...
export function KdvFilePopover({
  customerId,
  year,
  month,
  fileCount,
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/kdv-kontrol/customer-files?${params}`, {
  ...
  }, [customerId, year, month, fileCategory]);
  ...
  const handleTriggerClick = useCallback(async (e: React.MouseEvent) => {
    ...
    if (fileCount === 1) {
      ...
      try {
        ...
        console.error("[KdvFilePopover] Dosya açılamadı:", error);
    ...
    try {
      ...
      console.error("[KdvFilePopover] Dosyalar getirilemedi:", error);
      setNoFiles(true);
      setFiles([]);
    } finally {
      setLoading(false);
