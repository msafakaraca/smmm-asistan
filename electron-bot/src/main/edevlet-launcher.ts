/**
 * e-Devlet Kapısı Launcher
 * ========================
 * Puppeteer ile headed browser açarak e-Devlet Kapısı'na giriş yapar
 * ve T.C. Kimlik No + şifre alanlarını doldurur.
 * Giriş butonuna basmaz - kullanıcı kendisi yapar.
 */
...
export interface EdevletLaunchOptions {
  tckn: string;          // T.C. Kimlik No (11 hane)
  password: string;      // e-Devlet şifresi
  customerName?: string; // Mükellef adı (log için)
  onProgress: (status: string) => void;
}

export interface EdevletLaunchResult {
  success: boolean;
  browserOpen?: boolean;
  error?: string;
}
...
const EDEVLET_CONFIG = {
  LOGIN_URL: 'https://giris.turkiye.gov.tr/Giris/gir',
  ...
  SELECTORS: {
    // T.C. Kimlik No input
    TCKN: 'input#tridField',
    TCKN_ALT: 'input[name="tridField"]',
    // e-Devlet Şifresi input
    PASSWORD: 'input#egpField',
    PASSWORD_ALT: 'input[name="egpField"]',
  },
};
...
async function bringBrowserWindowToFront(): Promise<void> {
  if (process.platform !== 'win32') {
    console.log('[EDEVLET-LAUNCHER] ℹ️ bringBrowserWindowToFront sadece Windows\'ta çalışır');
    return;
  }
...
`;
  ...
  return new Promise((resolve) => {
    exec(`powershell -ExecutionPolicy Bypass -Command "${psScript.replace(/"/g, '\\"').replace(/\$/g, '`$')}"`, (error) => {
      if (error) {
        console.warn('[EDEVLET-LAUNCHER] PowerShell bringToFront hatası:', error.message);
      } else {
        console.log('[EDEVLET-LAUNCHER] ✅ Chrome penceresi ön plana getirildi');
      }
      resolve();
    });
  });
  ...
  if (activeBrowser) {
    try {
      if (activeBrowser.connected) {
        console.log('[EDEVLET-LAUNCHER] Önceki tarayıcı kapatılıyor...');
        await activeBrowser.close();
      }
    } catch {
      console.log('[EDEVLET-LAUNCHER] Önceki tarayıcı zaten kapalı');
    }
    activeBrowser = null;
  }
}
...
async function fillInput(page: Page, selectors: string[], value: string, fieldName: string): Promise<boolean> {
  for (const selector of selectors) {
    try {
      // Timeout artırıldı: yavaş ağlar için 5 saniye
      ...
      if (success) {
        console.log(`[EDEVLET-LAUNCHER] ✅ ${fieldName} girildi (${selector})`);
        return true;
      }
    } catch {
  ...
  console.warn(`[EDEVLET-LAUNCHER] ⚠️ ${fieldName} input bulunamadı!`);
  return false;
}
...
/**
 * e-Devlet Kapısı'na giriş yapan ana fonksiyon
 * T.C. Kimlik No ve şifreyi doldurur, giriş butonuna BASMAZ
 */
export async function launchEdevletKapisi(options: EdevletLaunchOptions): Promise<EdevletLaunchResult> {
  ...
  try {
    ...
    console.log(`[EDEVLET-LAUNCHER] Chromium path: ${chromiumPath || 'Puppeteer varsayılanı'}`);
    ...
    if (mainWindow && mainWindow.isVisible() && !mainWindow.isMinimized()) {
      console.log('[EDEVLET-LAUNCHER] Electron penceresi minimize ediliyor...');
      mainWindow.minimize();
    ...
    console.log(`[EDEVLET-LAUNCHER] Chromium başlatıldı, PID: ${browserPid}`);
    ...
    browser.on('disconnected', () => {
      console.log('[EDEVLET-LAUNCHER] Tarayıcı kapandı');
    ...
    try {
      ...
      console.log('[EDEVLET-LAUNCHER] ✅ Pencere maximize edildi (CDP)');
    } catch (e) {
      console.warn('[EDEVLET-LAUNCHER] CDP maximize başarısız:', e);
    }
    ...
    onProgress('e-Devlet Kapısı giriş sayfası yükleniyor...');

    // Login sayfasına git
    await page.goto(EDEVLET_CONFIG.LOGIN_URL, {
      waitUntil: 'domcontentloaded',
      timeout: EDEVLET_CONFIG.TIMEOUTS.PAGE_LOAD,
    });
    ...
    await new Promise(r => setTimeout(r, EDEVLET_CONFIG.DELAYS.FORM_READY));

    onProgress('Giriş bilgileri dolduruluyor...');

    // T.C. Kimlik No gir
    const tcknSelectors = [
      EDEVLET_CONFIG.SELECTORS.TCKN,
      EDEVLET_CONFIG.SELECTORS.TCKN_ALT,
      'input[type="number"][maxlength="11"]',
    ];

    const tcknFilled = await fillInput(page, tcknSelectors, tckn, 'T.C. Kimlik No');

    await new Promise(r => setTimeout(r, EDEVLET_CONFIG.DELAYS.BETWEEN_INPUTS));

    // e-Devlet Şifresi gir
    const passwordSelectors = [
      EDEVLET_CONFIG.SELECTORS.PASSWORD,
      EDEVLET_CONFIG.SELECTORS.PASSWORD_ALT,
      'input[type="password"]',
    ];

    const passwordFilled = await fillInput(page, passwordSelectors, password, 'e-Devlet Şifresi');
    ...
    console.error('[EDEVLET-LAUNCHER] Hata:', errorMessage);
 ...
 */
export async function closeEdevletBrowser(): Promise<void> {
  await cleanupPreviousBrowser();
}
